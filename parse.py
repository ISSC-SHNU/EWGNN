...
def parse_method(args, n, c, d, device):
    if args.method == 'difformer':
        model=EWGNN(d,args.hidden_channels, c, num_layers=args.num_layers, alpha=args.alpha, dropout=args.dropout, num_heads=args.num_heads, use_bn=args.use_bn,
                    use_residual=args.use_residual, use_graph=args.use_graph, use_weight=args.use_weight, eta_mode=args.eta_mode, fixed_eta=args.fixed_eta).to(device)
    else:
...
def parser_add_main_args(parser):
    ...
    parser.add_argument('--data_dir', type=str, default='../../DIFFormer-main/data/')
    ...
    parser.add_argument('--sigma', type=float, default=0.5, help='weight for wave')
    ...
    parser.add_argument('--eta_mode', type=str, default='adaptive', choices=['adaptive', 'fixed'])
    parser.add_argument('--fixed_eta', type=float, default=0.1)
    
    # training
    parser.add_argument('--lr', type=float, default=0.01)
    parser.add_argument('--lr_sgd', type=float, default=1e-5)
    parser.add_argument('--weight_decay', type=float, default=1e-3)
    parser.add_argument('--dropout', type=float, default=0.0)
    parser.add_argument('--dropout1', type=float, default=0.4,
                        help='dropout ratio for the first dropout')
    parser.add_argument('--dropout2', type=float, default=0.4,
                        help='dropout ratio for the second dropout')
