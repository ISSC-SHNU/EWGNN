...
for run in range(args.runs):
    ...
    model.reset_parameters()
    adam_optimizer = torch.optim.Adam(model.parameters(), weight_decay=args.weight_decay, lr=args.lr)
    ...
    for epoch in range(args.epochs):
        ...
        out, embeddings = model(dataset.graph['node_feat'], dataset.graph['edge_index'], return_embeddings=True)
        if args.dataset in ('yelp-chi', 'deezer-europe', 'twitch-e', 'fb100', 'ogbn-proteins'):
    ...
    final_layer_embedding = best_embeddings[-1].detach().cpu().numpy()
