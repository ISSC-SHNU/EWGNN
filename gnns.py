...
class GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
        ...
        # 添加一个线性层将输入特征维度转换为 hidden_dim
        self.input_transform = nn.Linear(in_channels, hidden_channels)
        self.input_bn = nn.BatchNorm1d(hidden_channels)  # 初始特征转换后的 BN

        self.convs = nn.ModuleList()
        self.convs.append(
            GCNConv(hidden_channels, hidden_channels, cached=not save_mem))  # 第一层卷积

        self.bns = nn.ModuleList()
        self.bns.append(nn.BatchNorm1d(hidden_channels))  # 第一层卷积后的 BN
        for _ in range(num_layers - 2):
            self.convs.append(
                GCNConv(hidden_channels, hidden_channels, cached=not save_mem))  # 隐藏层卷积
            self.bns.append(nn.BatchNorm1d(hidden_channels))

        self.convs.append(
            GCNConv(hidden_channels, out_channels, cached=not save_mem))  # 输出层
    ...
    def forward(self, x, edge_index, return_embeddings=False):
        ...
        if return_embeddings:
            return x_out, embeddings  # 返回最终输出和隐藏层嵌入
        return x_out
...
class GAT(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
        ...
        # 添加一个线性层将输入特征维度转换为 hidden_dim * heads
        self.input_transform = nn.Linear(in_channels, hidden_channels * heads)
        self.input_bn = nn.BatchNorm1d(hidden_channels * heads)  # 初始特征转换后的 BN

        self.convs = nn.ModuleList()
        self.convs.append(
            GATConv(hidden_channels * heads, hidden_channels, dropout=dropout, heads=heads, concat=True))  # 第一层卷积

        self.bns = nn.ModuleList()
        self.bns.append(nn.BatchNorm1d(hidden_channels * heads))  # 第一层卷积后的 BN
        for _ in range(num_layers - 2):
            self.convs.append(
                GATConv(hidden_channels * heads, hidden_channels, dropout=dropout, heads=heads, concat=True))  # 隐藏层卷积
            self.bns.append(nn.BatchNorm1d(hidden_channels * heads))

        self.convs.append(
            GATConv(hidden_channels * heads, out_channels, dropout=dropout, heads=out_heads, concat=False))  # 输出层
...
######################################
# 1) Define an MGC preprocessing class (PyG version)
######################################
class MGCPrePyG(nn.Module):
    """
    Reproduce the logic of the original MGCPre:
    1) Multiply the adjacency matrix multiple times to obtain multi-order information
    2) At each step, perform element-wise max() aggregation into Am
    3) Finally use Am * 1.2 as adjacency, and multiply with x to get single-path features
    """

    def __init__(self, n_layers: int, edge_index: torch.Tensor, x: torch.Tensor, mink: int = 1):
        super(MGCPrePyG, self).__init__()
        self.n_layers = n_layers
        self.edge_index = edge_index
        self.x = x
        self.mink = mink  # May be hardcoded as 1 in the original code
        self.num_nodes = x.size(0)

    def process(self):
        """
        Returns a list containing only one feature tensor [x2],
        where x2 = (Am * x), and Am is a single adjacency matrix obtained
        through multiple multiplications + max aggregation.
        """
        # 1) Convert to Dense adjacency
        #   Original MGCPre uses batchA.to_dense(); here we use PyG's to_dense_adj.
        #   shape: (N, N)
        A_dense = to_dense_adj(self.edge_index, max_num_nodes=self.num_nodes).squeeze(0).to(self.x.device)

        # 2) Initialize: A = I (N×N), Am = 0 matrix
        N = self.num_nodes
        A = torch.eye(N, device=self.x.device)
        Am = torch.zeros_like(A)  # Used to record max

        # 3) Multiple multiplications + max
        #   Same approach as the original creta_A_dense(n_layers, adj, mink):
        #   for ell in range(n_layers), A = adj*A, if ell>=mink => max
        for ell in range(self.n_layers):
            A = torch.mm(A_dense, A)  # The (ell+1)th multiplication
            if ell >= self.mink:
                # Perform element-wise max between A and current Am
                stacked = torch.stack([Am, A], dim=0)  # shape [2, N, N]
                Am, _ = torch.max(stacked, dim=0)  # shape [N, N]

        # 4) Scale by 1.2 (similar to original code)
        Am = Am * 1.2

        # 5) Final feature x2 = (Am * x)
        x2 = torch.mm(Am, self.x)  # (N, d)

        # Return a list containing only x2, making it easy for MGC model to take [x2] directly in forward
        return [x2]


######################################
# 2) Define an MGC model class
######################################
class MGCModel(nn.Module):
    def __init__(self,
                 num_features: int,
                 num_classes: int,
                 hidden_dim: int,
                 steps: int = 2,
                 alpha=0.05,
                 dropout1=0.4,  # Note: default values required
                 dropout2=0.4):  # Same as above
        super(MGCModel, self).__init__()
        self.steps = steps
        ...
        # Use ModuleList to store the first 'steps' linear layers (one for each feature path)
        self.linears = nn.ModuleList()
        self.bns = nn.ModuleList()
        for _ in range(self.steps):
            self.linears.append(nn.Linear(self.num_features, self.hidden_dim, bias=True))
            self.bns.append(nn.BatchNorm1d(self.hidden_dim))

        # Final output layer
        self.linears.append(nn.Linear(self.hidden_dim, self.num_classes, bias=False))
        ...
        # coffs = [1-alpha, alpha]
        # If you want to make it learnable, set requires_grad=True
        coffs_np = np.array([1 - alpha, alpha])
        self.coffs = nn.Parameter(torch.from_numpy(coffs_np).float(), requires_grad=False)

    def reset_parameters(self):
        """
        Reset all parameters,
        to ensure multiple runs or training sessions start from the same initial state.
        """
        # Apply default reset to linear layers
    ...
    def forward(self, xs):
        """
        xs is a list: [ x_0, x_1, ..., x_{steps-1} ]
        """
        sumx = 0
        for i, x in enumerate(xs):
            x = self.dropout1(x)
            x = self.linears[i](x)  # Linear
            x = self.activation(x)  # ReLU
            sumx += self.coffs[i] * x  # Weighted fusion

        # Final layer
        x = self.dropout2(sumx)
        x = self.linears[-1](x)
        return x
